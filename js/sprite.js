...
function spriteMgr(options) {
	...
	this.update = function(percentage,loop_count){
		for (t in this.spriteSet) {
			var s = this.spriteSet[t];
			s.update_sync(percentage,loop_count);
		}
	};
	this.draw = function() {
		//clear canvas
...
function sprite(options, id, sprMgr) {
	...
	this.update_sync = function(percentage,loop_count) {
		var freq = 1/period;
		frameIndex = Math.floor(percentage*numberOfFrames*freq + numberOfFrames*freq*(loop_count%period));
